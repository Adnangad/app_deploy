...
from models.area import Area  # If this is needed

def initialize_db():
    """Function to populate the database with initial stock items."""
    stock_items = [
    ...
    for stock in stock_items:
        if stock.product not in existing_stocks:
            storage.new(stock)
            existing_stocks.add(stock.product)
            added = True  # Mark that we added at least one item

    if added:
        storage.save()
        print("Database initialized with new stock items.")
    else:
        print("No new stock items added, database already initialized.")
